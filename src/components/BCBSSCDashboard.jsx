...
function StatCards() {
  ...
  const clientFacing = bcbssc.events.filter(e => e.type !== 'Internal').length;
  const internal = bcbssc.events.filter(e => e.type === 'Internal').length;
  const cards = [
    ...
    { label: 'Touchpoints', value: String(bcbssc.events.length), subEl: <span><span style={{ color: '#12805c', fontWeight: 700 }}>● {clientFacing} client-facing</span>{'  '}<span style={{ color: '#6e6e6e' }}>● {internal} internal</span></span> },
  ];
  return (
